"""
Re-apply optimized get_geoplaces_for_tiles function.

Migration 0038 was modified after initial application, so the database
still runs the old version. This migration re-applies the current optimized
function with all improvements.
"""

from django.db import migrations

# Import the function SQL from 0038 to keep it DRY
# We can't import from 0038 directly (migrations aren't importable that way),
# so we reproduce the full function here.

OPTIMIZED_FUNCTION = """
-- Ensure optimization indexes exist (IF NOT EXISTS makes this safe to re-run)
...
-- Note: function would need to be restored from 0037 for full reverse
"""


class Migration(migrations.Migration):
    dependencies = [
        ("geometries", "0038_optimize_tile_function"),
    ]
