...
provider_registry.register(WikimediaCommonsProvider())
...
def nearby_images(
    ...
    # Step 1: Find GeoPlaces and Huts within requested radius
    ...
    # Search at the full requested radius (max 50 results)
    # Search for GeoPlaces
    geoplaces = list(
        GeoPlace.objects.filter(
            is_active=True,
            is_public=True,
            location__distance_lte=(query_point, D(m=radius)),
    ...
    logger.debug(
        f"Search radius {radius}m: Found {len(geoplaces)} GeoPlaces, {len(huts)} Huts"
    )
    ...
    if geoplaces or huts:
        ...
        logger.info(
            f"Found {len(geoplaces)} GeoPlaces and {len(huts)} Huts within {radius}m"
        )
    else:
        logger.info(f"No GeoPlaces or Huts found within {radius}m")
    ...
    # Step 4: Sort by provider priority first, then by distance
    PRIORITY = {
        "wodore": 1,
        "camptocamp": 2,
        "wikidata": 3,
    ...
    results.sort(key=lambda r: (PRIORITY.get(r.provider, 99), r.distance_m))
    logger.debug("✓ Sorted by provider priority and distance")
