...
class Command(BaseCommand):
    help = "Initialize and drop data in HutType table"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--save",
            action="store_true",
            help="Save entries from database as defaults (overwrite fixtures/huttypes.yml)",
        )
        parser.add_argument(
    ...
    def handle(self, drop: bool, fill: bool, all: bool, save: bool, ignore_media: bool, *args, **options):
        ...
        asset_path = "/huts/types"
        media_dst = os.path.relpath(f"{settings.MEDIA_ROOT}/{asset_path}")
        app_root = os.path.join(os.path.dirname(__file__), "..", "..")
        media_src = os.path.relpath(f"{app_root}/media/{asset_path}")
        if drop or all:
            db = HutType.objects
            self.stdout.write(f"Drop {db.count()} entries from table 'HutType'")
            db.all().delete()
            if not ignore_media and os.path.exists(media_dst):
                self.stdout.write(f"Remove media file folder '{media_dst}'")
                shutil.rmtree(media_dst)
        if fill or all:
            self.stdout.write(f"Load data from 'huttypes.yaml' fixtures")
            try:
                call_command("loaddata", "huttypes", app_label="huts")
