...
The system SHALL provide a `GeoPlaceOperation` model linking a GeoPlace to a `service/` Category (standard, reduced) with: nullable `capacity` (semantics per place type — beds/seats/visitors), a free-text `hours` string (OSM-compatible or custom, display-only), an `extra` JSON field, and `is_active`, unique per `(geo_place, relation)`.
...
The system SHALL migrate all `AmenityDetail` data into `GeoPlaceOperation` via a `migrate_amenity_to_operation` management command (fuzzy month values mapped: no=0, noish=25, maybe=50, yesish=75, yes=100, unknown=NULL) and SHALL delete the `AmenityDetail` model, its schema, imports and admin references afterwards.
