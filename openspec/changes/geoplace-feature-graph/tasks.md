...
- [ ] 1.4 Create `relation_categories` fixture: `relations/` (part_of, near, serves, access_point), `service/` (standard, reduced), `link_types/` (website, booking, social, phone), `brand/`; verify idempotent loaddata
...
- [ ] 2.3 `GeoPlaceRelation` model in `_associations.py`: from/to FKs (outgoing/incoming_relations), relation FK limited to `relations/` children, confidence, extra, is_active; unique triple + no-self-loop constraints; composite indexes; makemigrations `create_geoplacerelation`
...
- [ ] 3.6 `geometries/admin/_geoplace.py`: remove AmenityDetail, register GeoPlaceRelation/GeoPlaceOperation inlines or admins (repo admin conventions: unfold, limit_choices_to, autocomplete)
...
- [ ] 5.1 Tests: models (constraints, MonthAccessor round-trip/errors), helpers (add_relation idempotency, direction lookup), fixtures idempotency, month-percentage query; run `inv tests`
